...
Eg：以矩阵为例，共有两个轴（行和列/竖轴和横轴）：

1. 0轴沿着0行到1行的方向垂直向下（竖轴：列）
2. 1轴沿着0列到1列的方向水平延伸（横轴：行）

若axis=-1，即按横轴求和（行：每行元素进行求和））即：`[0+1+2+3, 4+5+6+7]` ==> `[6, 22]`
```
X = np.array([
    [0, 1, 2, 3],
    [4, 5, 6, 7]])
...
2.2 如果**axis取正数**（此处取0），矩阵含有两个轴，第一个轴为0轴（纵向：列）。

Eg：若axis=0，则表示以**第一个轴（0轴，因为索引从0开始)** 的元素进行求和。本例为按列（第一个轴）进行求和（按竖轴求和）,即`[0+4, 1+5, 2+6, 3+7]` ==> `[4 6 8 10]`
```
X = np.array([
    [0, 1, 2, 3],
    [4, 5, 6, 7]])
...
Eg：若axis=(1, 0)，则表示先以第二个轴（1轴）即**按行求和**，得`[6, 22]`。变为了一维向量，再以第一个轴（0轴），因为成**一维向量**只有一个轴，所以也为按行求和即`6+22`，得28。
```
X = np.array([
    [0, 1, 2, 3],
    [4, 5, 6, 7]])
